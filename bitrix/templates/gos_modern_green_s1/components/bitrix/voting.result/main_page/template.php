...
?>
    ...
    <div class="col col-mb-12 interview-wrap white-box">
        <div class="interview">
            <h1>Опрос</h1>
            ...
            foreach ($arResult["QUESTIONS"] as $arQuestion):
                $iCount++;
                ?>
                <ol class="vote-items-list vote-answers-list" style="padding: 10px;">
                    <?
                    foreach ($arQuestion["ANSWERS"] as $arAnswer):
                        ...
                        if ($arParams["THEME"] == ""):
                            ...
                            <div class="graph-bar-wrap">
                                <div class="graph-bar" style="width: <?= $arAnswer["BAR_PERCENT"] ?>%;">&nbsp;</div>
                            </div>
                            <? if (isset($arResult['GROUP_ANSWERS'][$arAnswer['ID']])): ?>
                            ...
                            <? foreach ($arGroupAnswers as $arGroupAnswer): ?>
                                ...
                                <? } ?>
                                <?= $arGroupAnswer["MESSAGE"] ?> - <?= ($arGroupAnswer["COUNTER"] > 0 ? '&nbsp;' : '') ?><?= $arGroupAnswer["COUNTER"] ?> (<?= $arGroupAnswer["PERCENT"] ?>%)
                                <br/>
                                <div class="graph-bar-wrap">
                                    <div class="graph-bar" style="width: <?= $arGroupAnswer["PERCENT"] ?>%;">
                                        &nbsp;</div>
                                </div>
                            <? endforeach ?>
                        <? endif; // GROUP_ANSWERS
                            ...
                            <? foreach ($arGroupAnswers as $arGroupAnswer): ?>
                                ...
                                <div class="graph">
                                    <nobr class="bar" style="width: <?= (round($arGroupAnswer["PERCENT"])) ?>%;">
                                        <span><?= $arGroupAnswer["COUNTER"] ?> (<?= $arGroupAnswer["PERCENT"] ?>
                                            %)</span>
                                    </nobr>
                                </div>
                            <? endforeach ?>
